...
static void collect_emit(const Poly *p, const Coord *hidden, int hidden_count, void *userdata) {
    ...
    if (!sv_contains(&ctx->levels[hidden_count], &s)) {
        Poly key;
        sv_push(&ctx->levels[hidden_count], &s);
        poly_hash_key(p, ctx->lattice, &key);
...
int main(int argc, char **argv) {
    ...
    Poly seed_raw, seed_key;
    seed_raw.cycle_count = 1;
    seed_raw.cycles[0] = tile.base;
    poly_hash_key(&seed_raw, tile.lattice, &seed_key);

    StateVec levels[MAX_LEVELS];
    HashTable poly_seen[MAX_LEVELS];
    ...
    EmitCtx ectx = { levels, poly_seen, tile.lattice, max_n };

    for (int level = 0; level <= max_n; level++) {
        if (level > 0) printf("n=%d count=%zu\n", level, poly_seen[level].count);
