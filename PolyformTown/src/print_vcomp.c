...
typedef struct { StateVec *levels; int lattice; int max_level; } EmitCtx;
...
static void collect_emit(const Poly *p,const Coord *hidden,int hidden_count,void *userdata){ EmitCtx *ctx=userdata; if(hidden_count>ctx->max_level) return; State s; s.poly=*p; s.hidden_count=hidden_count; for(int i=0;i<hidden_count;i++) s.hidden[i]=hidden[i]; qsort(s.hidden,s.hidden_count,sizeof(Coord),coord_cmp); if(!sv_contains(&ctx->levels[hidden_count],&s)) sv_push(&ctx->levels[hidden_count],&s); }

int main(int argc,char **argv){ int max_n=5; const char *tile_path="tiles/monomino.tile"; if(argc>1) max_n=atoi(argv[1]); if(argc>2) tile_path=argv[2]; if(max_n<0) max_n=0; if(max_n>=MAX_LEVELS) max_n=MAX_LEVELS-1; Tile tile; if(!tile_load(tile_path,&tile)){ fprintf(stderr,"failed to load tile: %s\n",tile_path); return 1; } Poly seed_raw, seed; seed_raw.cycle_count=1; seed_raw.cycles[0]=tile.base; poly_canonicalize_lattice(&seed_raw,&seed,tile.lattice); StateVec levels[MAX_LEVELS]; for(int i=0;i<=max_n;i++) sv_init(&levels[i]); State seed_state; memset(&seed_state,0,sizeof(seed_state)); seed_state.poly=seed; sv_push(&levels[0],&seed_state); EmitCtx ectx={levels,tile.lattice,max_n}; for(int level=0; level<=max_n; level++){ if(level==max_n){ HashTable printed; hash_init(&printed,1024); for(size_t i=0;i<levels[level].count;i++){ Poly canon; poly_canonicalize_lattice(&levels[level].data[i].poly,&canon,tile.lattice); if(hash_insert(&printed,&canon)) tile_print_imgtable_shape(&tile,&canon); } hash_destroy(&printed); break; } for(size_t i=0;i<levels[level].count;i++){ Coord verts[MAX_VERTS*MAX_CYCLES]; int vc=build_boundary_vertices(&levels[level].data[i].poly, verts); for(int j=0;j<vc;j++){ enumerate_vertex_completions(&levels[level].data[i].poly,&tile,verts[j],levels[level].data[i].hidden,levels[level].data[i].hidden_count,collect_emit,&ectx); } } } for(int i=0;i<=max_n;i++) sv_destroy(&levels[i]); return 0; }
