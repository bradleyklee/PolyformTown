...
`--grouped` emits one labeled block per completion:
`Aggregate` boundary, `Tiles` used in the completion, and `Center`
for the shared completed vertex.

RL0 generation now writes only live-boundary completions.
`rl0_depict --live-only` remains accepted but should not change output
for fresh data generated by `rl0_generate`.
